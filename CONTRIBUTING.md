...
4. Check the blast radius: `python3 scripts/build-index.py --blast <spec-slug> <a few words the old behaviour used>`. It lists every document that links to the spec, every code comment that cites it, and every line of prose that uses those words, in the README, the site, the onboarding page and the store listing. Fix each one that no longer tells the truth.
5. Then change the code in `extension/`.
6. Run `python3 scripts/build-index.py`. It regenerates `docs/index.md` and fails on broken links.
7. Test on a real iPad with an Apple Pencil. The simulator cannot produce Pencil pressure, and a Mac has no Pencil at all. Tick the spec's Acceptance items you verified in the pull request description.

Spec, blast radius and code land in the same pull request. A pull request that changes behaviour without touching a spec is sent back.
